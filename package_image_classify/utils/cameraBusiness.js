...
// 加载文字模型（只支持英文字母）
function loadTextModel(message, callback) {
    const loader = new THREE.FontLoader();
    loader.load(fontUrl, function (font) {
        fontFace = font
        const model = createText(message)
        mainModel = model
...
// isAddModel:是否将3D模型加入到threejs场景
// model: 3D模型
function addModelByHitTest(evt, resetPanel, isAddModel, model) {
    // 点击可移动3D模型位置
    ...
    if (touches.length === 1) {
        ...
        if (hitTestRes && hitTestRes.length) {
            ...
            // 将矩阵分解到平移position、旋转quaternion，但不修改缩放scale。
            model.matrix.decompose(model.position, model.quaternion, new THREE.Vector3())
            console.log('addModelByHitTest', model.position)
            // 让模型model朝向相机camera的位置
